...
public class DateUtil {
  ...
  public static String formatTimeBetween(Long start, Long end) {
    long betweenMs = end - start;
    if (betweenMs > 0L) {
      final String retFormat = "{0}天{1}小时{2}分钟";
      long nd = 1000 * 24 * 60 * 60; // 一天的毫秒数
      long nh = 1000 * 60 * 60; // 一小时的毫秒数
      long nm = 1000 * 60; // 小时计算变量
      long day = betweenMs / nd;
      long hour = betweenMs % nd / nh + day * 24;
      long min = betweenMs % nd % nh / nm;
      return MessageFormat.format(retFormat, day, hour, min);
    } else {
      return null;
    }
  }
